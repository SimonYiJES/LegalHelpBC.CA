<?php
function gavias_uxima_render_block($key) {
...
function gavias_uxima_makeid($length = 5){
...
function gavias_uxima_base_url(){
  global $base_url;
  $theme_path = drupal_get_path('theme', 'gavias_uxima');
  return $base_url . '/' . $theme_path . '/';
}

global $gva_node_index;
function gavias_uxima_preprocess_node(&$variables) {
  global $gva_node_index;
...
function gavias_uxima_preprocess_node__portfolio(&$variables){
  $node = $variables['node'];
...
function gavias_uxima_preprocess_node__event(&$variables){
  $node = $variables['node'];
  $event_date = array();
...
function gavias_uxima_preprocess_breadcrumb(&$variables){
